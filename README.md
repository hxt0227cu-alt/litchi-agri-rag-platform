...
- **Agent 能真正"做事"**：不只是查资料问答——农户说"拿不准，帮我联系门店"，Agent 自动匹配方案并创建真实求助工单，门店端立即可见，进入协同闭环。
- **SSE 实时逐步进度**：任务执行过程中通过 Server-Sent Events 实时推送每一步工具执行状态，前端显示"已执行 1/3 步：知识库检索 → 果园档案 → 发起求助"，而非干转圈。
- **受限 Agent 治理**：Planner + 权限过滤 + 顺序执行 + 结果综合四阶段架构，模型只能从白名单选工具，不能生成代码/SQL/URL；计划最多 4 步；六类安全意图拦截，固定安全评测 10/10 拒绝。
- **写工具幂等 + 审批**：两个写工具（农户自动求助 / 技术员方案落库），以运行 ID 为幂等键防重复；技术员写工具需人工审批后才落库，审批拒绝不执行。
- **三表持久化 + 跨实例恢复**：主表 + 步骤表 + 审批表拆分，后台 60 秒扫描恢复陈旧运行，等待审批的任务跨实例保留可继续决策。
- **多级降级不白屏**：MySQL / Neo4j / Milvus / Ollama 任一不可用时显式返回 degraded 状态并本地证据回退，100 并发聊天 P95 69.7ms。
...
### 2. 写工具：农户自动求助 + 技术员审批落库

平台提供两个写工具，让 Agent 不止于查资料，而是能真正改变系统状态：

- **`create_consultation`（农户可用，无需审批）**：当问题复杂、证据不足，或农户说“拿不准 / 联系门店”时，Agent 自动匹配门店方案并创建真实求助工单，门店端立即可见，进入协同处理闭环——这是 Agent 与普通问答的本质区别。
- **`pending_remedy_plan`（技术员可用，需审批）**：审批前只返回动作预览，技术员在 `waiting_approval` 状态确认后才落库；审批拒绝不执行写入。

为避免重复审批 / 重复恢复造成重复落库，写工具以 **Agent 运行 ID 作为幂等键**：`createPlan` 先查重，同一键重复提交直接返回既有方案，不产生重复记录；审批决策本身带 `decided_by` / `decided_at` 审计。幂等行为有专门单元测试覆盖。
...
- 两个写工具：`create_consultation`（农户自动发起求助）与 `pending_remedy_plan`（技术员处置方案，需审批确认）。
